...
The Vapour Pressure Deficit is calculated as the difference from the saturation vapour pressure (`e_s`) and the actual vapour pressure(`e_a`).
...
This calculation should be probably made is a more accurate way (likely take the mean of everything is not a great idea), but seems the gap filling performance is satisfying.

```{r, results = 'asis',  echo = FALSE}
knit_template("analysis/fragments/variable_assess_gapfilling.rmd", var = "VPD", unit="hPa")
```
...
```{r, results = 'asis',  echo = FALSE}
knit_template("analysis/fragments/variable_assess_gapfilling.rmd", var = "PA", unit="hPa")
```
