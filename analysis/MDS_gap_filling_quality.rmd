...
To have a simple test of the accuracy of the gap filling algorith artificials gaps are created and the Root Mean Square Error (RMSE) is computed.

For each of the variable of interest:

 - `r n_rep * 30` gaps with a different length (from 1 hour to 60 days) and random location are generated
...
knit_template("analysis/fragments/variable_assess_gapfilling.rmd", var = "Tair", unit="°C")
```
## SW_IN
...
The Vapour Pressure Deficit is calcuated as the difference from the saturation vapour pressure (`e_s`) and the actual vapour pressure(`e_a`).
...
summarize(vpd_acc = mean(vpd_acc) %>%  round(2))
```

This calculation should be probably made is a more accurate way (likely take the mean of everything is not a great idea), but seems the gap filling performance is satisfing.
```{r, results = 'asis',  echo = FALSE}
...
The Precipitation is an hard variable to gap fill, because is erratic and has a limited connection with the other variables.

The good overall RMSE value is due to the high number of observations where P is 0.
...
```
```{r, P_plot_rmse, fig.cap = "Average RMSE for dfferent gap lentgth. Shaded area is mean +/- std. dev"}
P_rmse_stat_n0 %>%
  mutate(gap_length = gap_length / 2) %>%
  ggplot(aes(gap_length)) +
  geom_ribbon(aes(ymin = error_low, ymax = error_high), alpha = .5) +
  geom_line(aes(y=mean)) +
  labs(x = "Gap Lengths [hours]", y = "RMSE [{{unit}}]", title = "P")
```
## PA
```{r, results = 'asis',  echo = FALSE}
