...
There is again a window of increasing size starting from 7 days.



## Gap generation

To have a simple test of the accuracy of the gap filling algorith artificial gaps are created and filled using `REddyProc`.

For each of the variable of interest:

 - **`r n_rep`** gaps for each of 30 different lengths (ranging from 1 hour to 60 days) at random location are generated (so `n_rep * 30` gaps in total)
 - the gaps are filled using `REddyProc` default settings


Note the gaps are created only for the variable of interest, not for the other variables (for gaps in multiple variables see [MDS_multiple_missing](MDS_multiple_missing_gap_filling_quality.html))

## Metric

The main metric is Root Mean Square Error (RMSE).

 - for each the gap period RMSE is calculated
 - for each gap length the average RMSE and the RMSE standard deviation is calculated

The RMSE is plotted in relation to the gap lengths and their distribution

Then the quality of the gap filling is performed visually using:

- scatter plot between measured and filled values
- distribution of measured vs filled values (for 4 sample gap lengths)
- timeseries of measured vs filled (for 4 sample gap lengths)
...
This is an ambitious goal, but gives the upper limit of the errors.
...
For basically all variables the RMSE increases with gap length and then stabilize on an average values.

For some variables the RMSE is several times the instrument accuracy ( from ~40 times for temperature to ~3 times for air pressure) and for toehre variables like VPD or precipitation the average RMSE is not a good metric due to the high number of 0s in the measuremnts.

The distributions of the gap filled measurements if usually good, for VPD and PA the occurence of the mean value is overestimated.


In general the error of the gap filling methods is important and can have a significant impact on the quality of the research that uses this data.


## Analysis notes

- The procedure to generate gaps is relatively simple. Can consider to use something like [`mice::ampute`]( https://rianneschouten.github.io/mice_ampute/vignette/ampute.html). First example of this analysis available at: [MDS_multiple_missing](MDS_multiple_missing_gap_filling_quality.html)
- This is the analysis only for one site, should include data from multiple site, possible to cover different ecosystems. Example here there is the analysis performed using data from [Hyytiälä (Finland)](MDS_gap_filling_quality_Hyy.html), and the results seems comparable.
- The RMSE is a simple metric to assess the quality of the gap filling as it doesn't take into account the shape of imputed time series (for example see https://proceedings.neurips.cc/paper/2019/file/466accbac9a66b805ba50e42ad715740-Paper.pdf )

Improvements:

- neeed to handle variables where there a lot of 0 values (precipitation, VPD, SW_IN). Maybe make metrics without 0? or plot the error dsitribution or plot the relation between the measurement value and the error
